...
export default async function BuyerQuoteDetailPage({ params }) {
  const buyer = await requireUser('buyer');
  const { id } = await params;
  const quote = await getQuote(id);

  if (!quote || quote.buyer_id !== buyer.id) {
    notFound();
  }
      ...
      <section className="page">
        <div className="page-heading">
          <div>
            ...
            <h1>Quote from {quote.seller_name}</h1>
          </div>
          <p className="page-note">
            Status: <span className="pill">{quote.status}</span>
            {isExpired && <span className="pill" style={{ marginLeft: '0.5rem', backgroundColor: '#fca5a5' }}>Expired</span>}
          </p>
        </div>
        ...
        <div className="grid two">
          <div className="panel stack">
            ...
            <p><strong>Name</strong><br /><span className="muted">{quote.seller_name}</span></p>
            <p><strong>Email</strong><br /><span className="muted">{quote.seller_email}</span></p>
          ...
          <div className="panel stack">
            ...
            <p><strong>Valid until</strong><br /><span className={isExpired ? 'error' : 'muted'}>{new Date(quote.expires_at).toLocaleDateString()}</span></p>
        ...
        <div style={{ marginTop: '2rem' }}>
          <Link href="/buyer/quotes" className="button secondary">Back to quotes</Link>
        </div>
      </section>
    </main>
  );
